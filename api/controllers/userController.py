...
class UserController:
    ...
    def login(cls):
        ...
        if User.is_registered(user):
            ...
            # Esto significa que la aplicación web recordará el nombre del usuario mientras la sesión esté activa.
    ...
    def crear_usuario(cls):
        ...
        user = User(
            ...
            user_status_status_id = data.get('user_status_status_id'),
            user_roles_role_id = data.get('user_roles_role_id')
        )
        # username = request.args.get('username')
        # password = request.args.get('password')
        # user = User(username=username, password=password)
        ...
        if User.is_registered(user):
            ...
            return {"message": "Este usuario fue creado con exito"}, 401
        else:
            print(f"\033[91m{user}\033[0m")
